...
const UserManagement = ({ setCurrentView }) => {
  ...
  const handleSaveSingleApproval = async () => {
    ...
    let res, insErr, updErr;

    if (existing) {
      // update existing record
      const { data: updatedData, error: updError } = await supabase
        .from('Single_Approval')
        .update({ allowed_to_approve: allowedToApprove })
        .eq('id', existing.id)
        .select()
        .single();
      updErr = updError;
      ...
      const { data: insertedData, error: insertError } = await supabase
        .from('Single_Approval')
        .insert([{ username: currentUsername, allowed_to_approve: allowedToApprove }])
        .select()
        .single();
      insErr = insertError;
      res = insertedData;
    ...
    if (insErr || updErr) {
      console.error('Error saving approval:', insErr || updErr);
  ...
  const saveNewUser = async () => {
    ...
    // Step 5: Insert new license key (without UserKey)
          ...
          {approverViewMode === 'single' && (
            <>
              <Form.Group controlId="usernameDisplay">
                ...
                <Form.Control
                  type="text"
                  value={currentUsername}    // get this from user info
                  readOnly
                />
              </Form.Group>
                ...
                <div className="d-flex flex-column gap-3">
                  {savedApprovals.map((approval) => (
                    ...
                    >
                      ...
                      <p><strong>Username:</strong> {approval.username}</p>
                      <p><strong>Approval:</strong> {approval.allowed_to_approve ? 'Yes' : 'No'}</p>
