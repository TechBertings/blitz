...
const Activity = () => {
  ...
  const fetchActivities = async () => {
    ...
    if (error) {
      Swal.fire('Error', 'Error fetching activities: ' + error.message, 'error');
    } else {
      setActivities(data);
    }
    setLoading(false);
  ...
  const generateNextCode = () => {
    if (activities.length === 0) return '10001';
    const nums = activities.map(act => {
      const n = parseInt(act.code, 10);
      return isNaN(n) ? 0 : n;
    });
              ...
              <tbody>
                {currentItems.length === 0 ? (
                  ...
                  <tr key={act.id} style={{ borderBottom: '1px solid #ddd' }}>
                    ...
                    <td style={tdStyle}>
                      <button onClick={() => openEditModal(act)} style={actionBtnStyle}>Edit</button>
                      <button onClick={() => handleDelete(act.id)} style={{ ...actionBtnStyle, backgroundColor: '#dc3545' }}>Delete</button>
                    </td>
                  </tr>
                ))}
              </tbody>
...
// === Styles (same as other components)
