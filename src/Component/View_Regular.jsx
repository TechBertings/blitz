...
function View_Regular({ selectedVisa, setCurrentView }) {
  ...
  const visaType = selectedVisa?.visaType || "Regular";
  ...
  const fetchRelatedData = async () => {
    try {
      const baseTable = `${visaType}_Visa`; // still dynamic for main visa record
    ...
    >
      <table
        style={{
          width: "100%",
          minWidth: "600px",
          borderCollapse: "separate", // Changed
          borderSpacing: "0 8px",      // Added spacing between rows (vertical)
  ...
  const formatLabel = (str) => {
    ...
    return str
      ...
      .replace(/([A-Z])([A-Z][a-z])/g, "$1 $2")
      .replace(/^./, (char) => char.toUpperCase())
      .replace(/Visa/gi, "PWP"); // Replace 'Visa' with 'PWP'
  };
  ...
  const formatCreatedAt = (isoString) => {
    ...
    if (isNaN(date)) return isoString; // fallback if invalid date
        ...
        {filteredKeys
          ...
          .map(([key]) => (
            ...
            >
              ...
              {/* Render all keys as input fields, including Created_at */}
