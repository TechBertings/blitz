...
export default function ApprovalsPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      // Only cover_pwp and regular_pwp
      const visaTables = ['cover_pwp', 'regular_pwp'];
      let combinedData = [];
      let allowedNames = [];

      // TODO: fetch your allowed approvers / userApprovers etc., to populate allowedNames
      // Example: 
      // const { data: approverData, error: approverError } = await supabase.from('User_Approvers').select('*');
      // if (!approverError) allowedNames = approverData.map(a => a.name);
      ...
      for (const table of visaTables) {
        ...
        const formatted = filteredData.map(item => {
          if (table === 'cover_pwp') {
            // For cover_pwp, you may already have fields like cover_code, etc.
            return {
              ...
              responseDate: '', // fill if you have a date field in cover_pwp
              sourceTable: table,
              created_at: item.created_at || 'N/A',
            };
          } else if (table === 'regular_pwp') {
            // For regular_pwp, based on your schema:
            return {
              code: item.regularpwpcode || '',
              title: item.pwptype || 'N/A',                // using pwptype
              type: item.accountType ? item.accountType.join(', ') : 'N/A',  // accountType is text[]
              company: item.distributor || 'N/A',
              principal: item.objective || 'N/A',
              brand: item.promoScheme || 'N/A',
              approver: item.approver || 'N/A',             // only if you have this field; I didn't see approver in your schema, so maybe this is always "N/A"
              createForm: item.CreatedForm || item.createForm || 'N/A',
              status: item.notification === true ? 'Approved' : 'Pending',
              responseDate: '', // if you have a field for when it was responded / updated
              sourceTable: table,
              created_at: item.created_at || 'N/A',
            };
          } else {
